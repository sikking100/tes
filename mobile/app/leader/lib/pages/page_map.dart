...
class LatLngNotifier extends StateNotifier<AsyncValue<LatLng>> {
  LatLngNotifier(BusinessAddress? address, this.ref) : super(const AsyncValue.loading()) {
    if (address != null) {
      ...
      ref.watch(textEditingController).text = address.name;
      return;
    } else {
      init();
    }
  ...
  void init() async {
    state = await AsyncValue.guard(() async {
      final defLoc = await Location.instance.getLocation();
      ...
      return const LatLng(-6.229728, 106.6894312);
    });
  }
  ...
  Future getData(LatLng latLng) async {
    final result = await placemarkFromCoordinates(latLng.latitude, latLng.longitude, localeIdentifier: 'en');
    final res =
    ...
    ref.watch(textEditingController).text = res;
...
class SuggestionMap extends StateNotifier<AsyncValue<List<Place>>> {
  ...
  Future<PlaceDetail> getDetails(String id) async {
    try {
      final result = await _api.maps.find(id);
      return result;
    } catch (e) {
      rethrow;
    }
                    ...
                    ElevatedButton(
                      ...
                      onPressed: loading
                          ...
                          : () async {
                              ...
                              // argument?.action!(BusinessAddress(name: textEdit.text, lngLat: [data.longitude, data.latitude]));
                              await Future.delayed(const Duration(seconds: 3));
                              if (context.mounted) Navigator.pop(context, 'ok');
                              ref.read(loadingStateProvider.notifier).update((state) => false);
                            },
                      child: loading ? const CircularProgressIndicator.adaptive() : const Text('Simpan'),
                    ),
                  ],
                ),
              ),
