...
final completeStateNotifierProvider = StateNotifierProvider.autoDispose<CompleteNotifier, PagingController<int, Order>>((ref) {
  return CompleteNotifier(ref);
});

class CompleteNotifier extends StateNotifier<PagingController<int, Order>> {
  CompleteNotifier(this.ref) : super(PagingController(firstPageKey: 1)) {
  ...
  void request(int pageKey) async {
    try {
      final result = await ref.read(apiProvider).order.find(query: '${user.id},2', num: pageKey);
      if (result.next == null) {
...
class ViewOrderComplete extends ConsumerWidget {
  const ViewOrderComplete({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(completeStateNotifierProvider);
