...
final waitPayStateNotifierProvider = StateNotifierProvider.autoDispose<WaitPayNotifier, PagingController<int, Order>>((ref) {
  return WaitPayNotifier(ref);
});

class WaitPayNotifier extends StateNotifier<PagingController<int, Order>> {
  WaitPayNotifier(this.ref) : super(PagingController(firstPageKey: 1)) {
  ...
  void request(int pageKey) async {
    try {
      final result = await ref.read(apiProvider).order.find(query: '${user.id},0', num: pageKey);
      if (result.next == null) {
...
class ViewOrderWaitPay extends ConsumerWidget {
  const ViewOrderWaitPay({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(waitPayStateNotifierProvider);
