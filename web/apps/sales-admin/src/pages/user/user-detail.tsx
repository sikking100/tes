...
import { TypeDetailCustomer, userStore } from './user'
import { customersService } from 'hooks'
import { getOrderApiInfo } from 'apis'
import { LoadingForm, Modals, PText, Shared } from 'ui'
import { Avatar, Box, Divider, HStack, SimpleGrid, Text } from '@chakra-ui/react'
import { WrapCustomerApplys } from '../business/customer-detail-apply'

interface PropsDetailCustomer {
    customerId: string
}

const DetailCustomer: React.FC<PropsDetailCustomer> = ({ customerId }: PropsDetailCustomer) => {
    ...
    const { data, isLoading, error } = customersService.useGetCustomerById(customerId)

    const findTransaction = async () => {
        try {
            const transactionLastMonth = await getOrderApiInfo().findTransactionLastMonth({ customerId })
    ...
    const onClose = () => {
        setUser(undefined)
        setUsers(undefined)
        setOpen({ isOpen: false, type: undefined })
    }
    ...
    const setOpenDetailBusines = () => {
        ...
        setOpen({ type: TypeDetailCustomer.DETAIL_BUSINESS, isOpen: true })
        setUser({
    ...
    return (
        <Modals isOpen={isOpen} setOpen={onClose} title="Detail Pelanggan" size="6xl">
                        ...
                        <HStack align={'start'}>
                            ...
                            <SimpleGrid columns={2} gap={5} w="full">
                                ...
                                <Box w="full">
                                    ...
                                    <Text textTransform={'capitalize'}>{Shared.formatRupiah(String(transactionLast))}</Text>
                                    <Divider />
                                </Box>
                                <Box w="full">
                                    <Text fontSize={'sm'} fontWeight={'bold'}>
