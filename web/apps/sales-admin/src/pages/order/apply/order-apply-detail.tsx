...
import { ButtonForm, entity, LoadingForm, Modals, PText, Tables, Types } from 'ui'
...
const OrderApplyDetail: React.FC<Props> = ({ isOpen, setOpen, id }) => {
    ...
    const { data: dataOrder, error: errorDataOrder, isLoading: isLoadingDataOrder } = orderService.useGetOrderById(`${id}`)
    ...
    const findCurrentApprover = () => {
        ...
        const find = data.userApprover.find((v) => v.id === admin.id && v.status === StatusOrderApply.WAITING_APPROVE)
        if (find) return true
        return false
    }
                    ...
                    <React.Fragment>
                        <HStack experimental_spaceX={8}>
                            <Box experimental_spaceY={2} fontWeight={'semibold'}>
                                <HStack>
                                    <Avatar size={'xl'} src={dataOrder.customer.imageUrl} name={dataOrder.customer.name} />
                                    <Box>
                        ...
                        <Divider my={2} />
                        <Tables pageH="100%" isLoading={isLoading} columns={column} data={isLoading ? [] : data.userApprover} />
                        {findCurrentApprover() && (
                            <Box>
                                <FormControl>
