...
const Wrap: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <Root appName="Sales" items={dataListOrder} backUrl={'/'} activeNum={3}>
        {children}
    </Root>
)

const OrderApplyy: React.FC<{ type: TypeOrderApply }> = ({ type }) => {
    const { column, isOpen, setIsOpen, id } = columns(type === TypeOrderApply.WAITING_APPROVE ? true : false)
    ...
    return (
        <Box mt={'5px'}>
            ...
            {error ? (
                ...
                <React.Fragment>
                    <Tables useTab={true} isLoading={isLoading} columns={column} data={isLoading ? [] : data} />
                </React.Fragment>
            )}
        </Box>
    )
